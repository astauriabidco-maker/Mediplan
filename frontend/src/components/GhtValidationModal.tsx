...
import { X, Check, XCircle, ShieldAlert, Loader2, Building } from 'lucide-react';
import { fetchShiftApplications, approveGhtApplication, rejectGhtApplication } from '../api/planning.api';
...
export const GhtValidationModal = ({ isOpen, onClose, onSuccess }: GhtValidationModalProps) => {
    ...
    const loadApplications = async () => {
        ...
        try {
            ...
            const pendingGht = data.filter(app => app.status === 'PENDING_GHT_APPROVAL');
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    {loading ? (
                        ...
                        <div className="text-center p-12 text-slate-400 p-12 flex flex-col items-center">
                            ...
                            <p className="text-lg">Aucune demande de déplacement inter-sites en attente.</p>
                        </div>
                    ) : (
                        applications.map(app => (
                            <div key={app.id} className="bg-slate-800/50 border border-slate-700 p-5 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="text-sm text-slate-400 flex flex-wrap gap-2">
                                        <span className="bg-slate-700/50 px-2 py-1 rounded">
                                            {format(new Date(app.shift?.start), "EEEE dd MMM yyyy 'à' HH:mm", { locale: fr })}
                                        </span>
                                        <span className="bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-1 rounded flex items-center gap-1">
                                            <ShieldAlert size={14} /> Déplacement Extérieur (Score {app.score})
                                        </span>
                                    </div>
                                    <div className="text-sm text-slate-500 mt-2">
                                        Du Poste : <span className="text-white">Site {app.agent?.facilityId}</span> → Vers : <span className="text-white">Site {app.shift?.facilityId} ({app.shift?.facility?.name || 'Inconnu'})</span>
                                    </div>
                                </div>
