...
export const AttendancePage = () => {
  ...
  const getStatusLabel = (status: AttendanceReport['status']) => {
    switch (status) {
      ...
      case 'EXTRA_SHIFT':
        return 'Heure Supp. (Non planifié)';
      case 'LATE':
        return 'En Retard';
      case 'ABSENT':
        return 'Absent';
      case 'PLANNED':
        return 'Prévu plus tard';
      default:
        return 'Repos';
    }
  };
  ...
  if (isLoading)
    return (
      <div className="p-8 text-white">
        Chargement des logs des pointeuses...
      </div>
    );
  ...
  let filteredReports =
    reports?.filter(
      (r) =>
        (r.agent.firstName?.toLowerCase() || '').includes(
          search.toLowerCase(),
        ) ||
        (r.agent.lastName?.toLowerCase() || '').includes(
          search.toLowerCase(),
        ) ||
  ...
  const presents =
    reports?.filter((r) => ['PRESENT_ON_TIME', 'LATE'].includes(r.status))
      .length || 0;
  ...
  const microLates =
    reports?.filter(
      (r) =>
        ...
        r.attendance.delayMinutes <= r.attendance.lateTolerance,
    ).length || 0;
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black text-white flex items-center gap-3">
            ...
            {isAgent ? 'Mes Pointages' : 'Tour de Contrôle : Assiduité'}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
            <div className="text-slate-400 text-sm font-medium mb-2 flex items-center gap-2">
              <Calendar size={16} /> Effectif Prévu
            </div>
          ...
          <div className="bg-amber-950 border border-amber-900/50 rounded-3xl p-6">
            <div className="text-amber-500 text-sm font-medium mb-2 flex items-center gap-2">
              <Clock size={16} /> Retards
            </div>
          ...
          <div className="bg-rose-950 border border-rose-900/50 rounded-3xl p-6">
            <div className="text-rose-500 text-sm font-medium mb-2 flex items-center gap-2">
              <UserX size={16} /> Absents (Non Signalé)
            </div>
          ...
          <div className="bg-indigo-950 border border-indigo-900/50 rounded-3xl p-6 relative overflow-hidden">
            <div className="text-indigo-400 text-sm font-medium mb-2 flex items-center gap-2 relative z-10">
              <AlertTriangle size={16} /> Micro-Retards (Furtifs)
            </div>
            <div className="text-4xl font-bold text-indigo-400 relative z-10">
              {microLates}
            </div>
            <div className="absolute -bottom-6 -right-6 text-indigo-500/10">
                  ...
                  <td className="py-4 px-4 text-slate-300">
                    {r.shift ? (
                      <div className="flex items-center gap-2">
                        ...
                        {format(new Date(r.shift.start), 'HH:mm')} -{' '}
                        {format(new Date(r.shift.end), 'HH:mm')}
                      </div>
                    ) : (
                    ...
                    {r.attendance.firstIn ? (
                      <div className="flex items-center gap-2">
                        <div className="font-mono text-emerald-400">
                          {format(new Date(r.attendance.firstIn), 'HH:mm:ss')}
                        </div>
                        {r.attendance.firstInSource === 'WHATSAPP' &&
                          r.attendance.firstInLocation && (
                            <a
