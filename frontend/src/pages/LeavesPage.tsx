      ...
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          <p className="text-slate-400">
            {safeFormat(leave.start, 'dd MMMM')} -{' '}
            {safeFormat(leave.end, 'dd MMMM yyyy')}
          </p>
...
function BalancesWidget({
  ...
  if (isLoading)
    return (
      <div className="h-24 rounded-3xl bg-slate-900/50 animate-pulse border border-slate-800" />
    );
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
      {balances.map((b) => {
        ...
        const percent = Math.min(
          100,
          Math.max(0, (remaining / b.allowance) * 100),
        );
        ...
        return (
          <div
            key={b.id}
            className="p-6 bg-slate-900 border border-slate-800 rounded-[2rem] flex flex-col justify-between hover:border-slate-700 transition-colors shadow-lg relative overflow-hidden group"
          >
            ...
            <div className="mt-6 relative z-10">
              ...
              <div className="w-full h-3 bg-slate-950 rounded-full overflow-hidden shadow-inner flex relative">
                <div
                  className={cn(
                    'h-full transition-all duration-1000 bg-current',
                    batteryColor,
                  )}
                  style={{ width: `${percent}%` }}
                />
      ...
      {!requests || requests.length === 0 ? (
        <div className="text-center p-20 bg-slate-900/40 rounded-3xl border-2 border-slate-800 border-dashed">
          ...
          <p className="text-slate-500 text-xl font-medium italic">
            Aucune demande en attente. Votre équipe est au complet !
          </p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {requests.map((req) => {
            ...
            return (
              <div
                key={req.id}
                className="bg-slate-900 border border-slate-800 rounded-3xl p-6 group hover:border-blue-500/30 transition-all duration-300 relative overflow-hidden flex flex-col justify-between"
              >
                ...
                <div>
                  ...
                  {(req.aiRecommendation || req.isAutoRejected) && (
                    <div
                      className={cn(
                        'mb-4 rounded-2xl p-4 border text-sm flex gap-3 backdrop-blur-sm',
                        aiRiskColor,
                      )}
                    >
                      <AlertCircle
                        className={cn('shrink-0 mt-0.5', aiIconColor)}
                        size={18}
                      />
                      <div>
                        <p className="font-bold mb-1 flex items-center gap-2">
                          ...
                          {req.aiScore !== undefined && (
                            <span
                              className={cn(
                                'px-2 py-0.5 rounded-md text-[10px] uppercase font-black bg-slate-950/50',
                              )}
                            >
                        ...
                        <p className="text-opacity-80 leading-snug">
                      ...
                      <div className="flex gap-2">
                        ...
                        <span>
                          ...
                          <span className="font-bold text-white">
                            {format(new Date(req.start), 'dd MMM yyyy', {
                              locale: fr,
                            })}
                          </span>{' '}
                          au{' '}
                          <span className="font-bold text-white">
                            {format(new Date(req.end), 'dd MMM yyyy', {
                              locale: fr,
                            })}
                          </span>
                        </span>
                      </div>
                      <span className="font-bold text-blue-400 bg-blue-500/10 px-2 py-1 rounded-lg">
                        {days} jours
                      </span>
                    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900/80 border border-white/10 w-full max-w-lg rounded-[2.5rem] p-10 space-y-8 shadow-2xl relative overflow-hidden backdrop-saturate-150">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-400">
            Soumettez votre absence pour validation.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4 pt-6">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-5 hover:bg-slate-800 text-slate-500 font-black rounded-2xl transition-all tracking-tight underline-offset-4 hover:underline"
            >
              Annuler
            </button>
            <button
            ...
            >
              ...
              Envoyer{' '}
              {formData.agentId !== currentAgentId
                ? "pour l'agent"
                : 'ma demande'}
            </button>
          </div>
...
export function LeavesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'my-leaves' | 'team-validation'>(
    'my-leaves',
  );
      ...
      <div className="flex flex-col items-center justify-center h-[60vh] gap-6 text-center animate-in zoom-in-95 duration-500">
        <div className="p-8 bg-rose-500/10 border-2 border-rose-500/20 rounded-[2.5rem] max-w-md shadow-2xl shadow-rose-500/5">
          ...
          <p className="text-slate-400 font-medium leading-relaxed">
            {(errorMyLeaves as any)?.message ||
              'Impossible de récupérer les données des congés pour le moment.'}
          </p>
          <button
    ...
    <div className="space-y-12 py-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex flex-col gap-4">
        ...
        <p className="text-slate-500 text-xl font-medium max-w-2xl leading-relaxed">
          Gérez vos absences, consultez vos soldes et gardez un œil sur la
          disponibilité de votre équipe au sein de l'HGD.
        </p>
      </div>
