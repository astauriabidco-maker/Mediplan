...
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, BarChart, Bar, Legend, Cell, LineChart, Line, PieChart, Pie, RadarChart, Radar, PolarGrid, PolarAngleAxis } from 'recharts';
import { TrendingUp, TrendingDown, Users, DollarSign, Activity, AlertCircle, Shield, Award, Heart, Sparkles, ChevronRight, X } from 'lucide-react';
...
export const DashboardPage = () => {
    ...
    const addWidget = (widget: any) => {
        ...
        if (dynamicWidgets.find(w => w.title === widget.title)) return;
        setDynamicWidgets(prev => [widget, ...prev].slice(0, 4));
    };

    const removeWidget = (title: string) => {
        ...
        return (
            <div className="p-8 flex items-center justify-center min-h-[400px]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-400 font-medium">Initialisation de l'Intelligence MediPlan...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-8 space-y-8 animate-[fadeIn_0.5s_ease-out] max-w-[1600px] mx-auto pb-20">
            ...
            {dynamicWidgets.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in slide-in-from-top-4 duration-500">
                    {dynamicWidgets.map((widget, i) => (
                        <div key={i} className="bg-slate-900 border border-blue-500/30 rounded-3xl p-6 relative group overflow-hidden shadow-2xl shadow-blue-500/10 hover:border-blue-500/50 transition-all">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button onClick={() => removeWidget(widget.title)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 transition-colors">
                                    <X size={16} />
                                </button>
                            </div>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    {widget.chartType === 'PIE' ? (
                                        <PieChart>
                                            ...
                                            <RechartsTooltip 
                                                contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '0.75rem', border: 'none' }}
                                                itemStyle={{ color: '#fff' }}
                                            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                <KpiCard title="Masse Salariale" value={formatCFA(kpis?.masseSalariale?.value || 0)} growth={kpis?.masseSalariale?.growth} icon={<DollarSign className="text-blue-500" />} color="blue" compact />
                <KpiCard title="Gardes & Supp." value={formatCFA(kpis?.coutHeuresSupp?.value || 0)} growth={kpis?.coutHeuresSupp?.growth} icon={<AlertCircle className="text-rose-500" />} color="rose" compact />
                <KpiCard title="Absentéisme" value={`${kpis?.tauxAbsentéisme?.value || 0}%`} growth={kpis?.tauxAbsentéisme?.growth} icon={<TrendingDown className="text-amber-500" />} color="amber" compact />
                <KpiCard title="Effectif Actif" value={kpis?.effectifActif?.value || 0} growth={kpis?.effectifActif?.growth} icon={<Users className="text-emerald-500" />} color="emerald" compact />
                <KpiCard title="Conformité GPEC" value={`${kpis?.gpecConformity?.value || 0}%`} growth={kpis?.gpecConformity?.growth} icon={<Award className="text-purple-500" />} color="purple" compact />
                <KpiCard title="Alertes Santé" value={kpis?.healthAlerts?.value || 0} growth={0} icon={<Shield className="text-red-500" />} color="red" compact isCritical={kpis?.healthAlerts?.value > 0} />
                <KpiCard title="Indice QVT" value={`${kpis?.qvtIndex?.value || 0}/100`} growth={kpis?.qvtIndex?.growth} icon={<Heart className="text-pink-500" />} color="pink" compact />
            </div>

            {/* Charts Row */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6">
                    <h2 className="text-xl font-bold text-white mb-6">Évolution Budgétaire (6 mois)</h2>
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={trends || []}>
                                <defs>
                                    <linearGradient id="colorMasseSalariale" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.8}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0}/></linearGradient>
                                </defs>
                                <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                                <XAxis dataKey="name" stroke="#94a3b8" tick={{fill: '#94a3b8'}} axisLine={false} tickLine={false} />
                                <YAxis stroke="#94a3b8" tick={{fill: '#94a3b8'}} axisLine={false} tickLine={false} tickFormatter={(val) => `${val / 1000000}M`} />
                                <RechartsTooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '0.75rem', color: '#fff' }} formatter={(value: any) => [formatCFA(Number(value) || 0), undefined]} />
                                <Area type="monotone" dataKey="masseSalariale" name="Masse Salariale Net" stroke="#3b82f6" fillOpacity={1} fill="url(#colorMasseSalariale)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                    <h2 className="text-xl font-bold text-white mb-6">Poids Budgétaire par Service</h2>
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={services || []} layout="vertical">
                                ...
                                <Bar dataKey="coutsGénérés" radius={[0, 4, 4, 0]} barSize={20}>
                                    {services?.map((_: any, index: number) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
...
const KpiCard = ({ title, value, growth, icon, color, compact, isCritical }: any) => {
    return (
        <div className={clsx(
            ...
            isCritical ? "border-rose-500/50 bg-rose-500/5" : "hover:border-slate-700 hover:bg-slate-800/50 shadow-lg"
        )}>
            ...
            <div className={clsx("font-black text-white relative z-10 tracking-tight", compact ? "text-xl" : "text-3xl")}>{value}</div>
            {growth !== undefined && (
                <div className="flex items-center gap-1 mt-2">
                    <span className={clsx(
                        ...
                        {growth > 0 ? <TrendingUp size={10} /> : (growth < 0 ? <TrendingDown size={10} /> : null)}{growth}%
                    </span>
                </div>
            )}
            <div className={clsx("absolute -right-4 -bottom-4 w-16 h-16 rounded-full blur-3xl opacity-10 group-hover:opacity-25 transition-opacity", `bg-${color}-500`)} />
        </div>
    );
};
