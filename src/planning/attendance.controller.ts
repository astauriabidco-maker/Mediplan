...
import { startOfDay, endOfDay, differenceInMinutes, isAfter } from 'date-fns';
...
export class AttendanceController {
  ...
  async syncFromHardware(
    ...
    if (!agent) {
      ...
      console.warn(
        `Badgeuse [${payload.deviceId}] : matricule inconnu ${payload.agentIdOrMatricule}`,
      );
  ...
  async getDailyStatus(
    ...
    let lateTolerance = await this.settingsService.getSetting(
      tenantId,
      null,
    ...
    const statusReport = agents.map((agent) => {
      const agentShifts = shifts.filter(
        (s) => s.postId === agent.id.toString(),
      );
      const agentAttendances = attendances.filter(
      ...
      const lastOut =
        logsOut.length > 0 ? logsOut[logsOut.length - 1].timestamp : null;
