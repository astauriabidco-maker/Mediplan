...
export class WhatsappController {
  ...
  handleIncoming(@Body() body: IncomingMessageDto) {
    ...
    if (
      ...
      this.whatsappService.handleIncomingMessage(body).catch((err: unknown) => {
        const message = err instanceof Error ? err.message : String(err);
        this.logger.error(`Error handling incoming message: ${message}`);
      });
    }
