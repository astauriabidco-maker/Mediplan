...
export class PayrollService {
  ...
  async generatePayslip(
    ...
    const shifts = await this.planningService.getShifts(
      tenantId,
      startOfMonth,
      endOfMonth,
    );
    ...
    for (const shift of agentShifts) {
      ...
      const shiftLogs = attendances.filter((a) => {
        const diffHours =
          Math.abs(a.timestamp.getTime() - shift.start.getTime()) /
          (1000 * 3600);
      ...
      let outPunch =
        outPunches.length > 0 ? outPunches[outPunches.length - 1] : null;

      if (!inPunch) {
        // ABSENT
        const shiftDuration =
          (shift.end.getTime() - shift.start.getTime()) / (1000 * 3600);
      ...
      const actualStart =
        inPunch.timestamp > shift.start ? inPunch.timestamp : shift.start; // S'il est en retard, on prend le chrono réel
      const actualEnd =
        outPunch.timestamp > shift.end ? shift.end : outPunch.timestamp; // On ne paie pas les débordements non justifiés par un "EXTRA"

      const durationHours = Math.max(
        0,
        (actualEnd.getTime() - actualStart.getTime()) / (1000 * 3600),
      );
      ...
      while (current < actualEnd) {
        ...
        const durHours =
          (nextHour.getTime() - current.getTime()) / (1000 * 3600);
        if (isNight) {
          nightHours += durHours;
    ...
    // 5. Moteur de règles dynamique avec évaluateur strict allowlist.
    ...
    for (const rule of rules) {
      try {
        ...
        if (rule.condition && rule.condition.trim() !== '') {
          if (!evaluatePayrollCondition(rule.condition, context)) {
            continue;
          }
        }
        ...
        const result = Math.round(
          evaluatePayrollFormula(rule.formula, context),
        );
        ...
        if (
          rule.type === PayrollRuleType.TAX ||
          rule.type === PayrollRuleType.DEDUCTION
        ) {
        ...
        console.error(`Echec de la règle ${rule.code}:`, errorMessage);
    ...
    const payslipDetails = payslip.details as Record<string, unknown>;
    payslipDetails.auditReport = auditReport;

    if (auditReport.anomalies.length > 0) {
      // Flag internal status visually for UI maybe
      // Using a loose boolean flag inside details
      payslipDetails.hasAnomalies = true;
    }
